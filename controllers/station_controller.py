...
class StationController:
    def _init_(self):
        ...
        self.charging_stations = {
            "Posto 1": {"x": 2.0, "y": 3.0, "ocupado": False, "tempo_expiracao": None,"id": None, "queue" : []},
            "Posto 2": {"x": 5.0, "y": 7.0, "ocupado": False, "tempo_expiracao": None,"id": None, "queue" : []},
            "Posto 3": {"x": 8.0, "y": 2.0, "ocupado": False, "tempo_expiracao": None,"id": None, "queue" : []},
            "Posto 4": {"x": 1.0, "y": 9.0, "ocupado": False, "tempo_expiracao": None,"id": None, "queue" : []},
            "Posto 5": {"x": 6.0, "y": 5.0, "ocupado": False, "tempo_expiracao": None,"id": None, "queue" : []},
        }
    ...
    def get_station_mais_proximo(self, x, y, id):
        ...
        try:
            ...
            if not postos_disponiveis:  # Se não houver postos disponíveis, retorna None
                menorTempo = 9999999
                for posto in self.charging_stations:
                    tempo = len(posto["queue"]) * 80 + calcular_distancia(x, y, self.charging_stations[posto]["x"], self.charging_stations[posto]["y"])
                    if tempo < menorTempo:
                        menorTempo = tempo
                        station_mais_proximo = posto
                
                self.set_station_occupied(station_mais_proximo,id)  #  Marca o posto como ocupado
                return {
                    "status": "Sucess",
                    "mensagem": "Posto reservado (fila) {posto}"
                }
    ...
    def get_stations_by_id(self, id_usuario):
        ...
        try:
            ...
            if stations_ocupadas:
                ...
                return {
                    "status": "sucesso",
                    "mensagem": f"Postos reservados: {station_mais_proximo}, {stations_ocupadas}"
                }
            else:
